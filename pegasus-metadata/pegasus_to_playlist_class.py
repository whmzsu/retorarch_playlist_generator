...
def convert_pic(pic_ori_file, file_fmt):
    im1 = Image.open(pic_ori_file)
    im1.save(os.path.splitext(pic_ori_file)[0] + "." + file_fmt)
...
def process_pic(game, rom_path, db_name, lplpathdir):
    ...
    elif game.background:
        ss_full_file = game.background.replace("media/", rom_path + "/media/")
    else:
        ss_full_file = ""
    if ss_full_file:
        ...
        if not ss_full_file.endswith(".png"):
            convert_pic(ss_full_file, "png")
            ss_full_file = os.path.splitext(ss_full_file)[0] + ".png"
...
def process_pic2(game, rom_path, db_name, lplpathdir):
    ...
    ss_full_file_jpg2 = os.path.join(
        rom_path, "media", gamefilename_woe, "boxfront.jpg"
    )
    ss_full_file_png2 = os.path.join(
        rom_path, "media", gamefilename_woe, "boxfront.png"
    )
...
def prepare_gamelists(sections, prefix, choice, rom_path, fix_db_name, lplpathdir):
    ...
    for gamedes in sections:
        ...
        if name:
            ...
            if game.screenshot or game.boxfront or game.logo or game.background:
...
def process(
    check1, entry_metadata_file, entry_dest_playlist, entry_prefix, entry_db_name
):
    choice = check1.get()
    ...
    gamelists = prepare_gamelists(
        sections, prefix, choice, rom_path, fix_db_name, lplpathdir
    )
...
def main():
    ...
    OS_type1 = tk.Radiobutton(
        frame1, text="游戏环境Windows系统", variable=check1, value=1, justify="left"
    )
    OS_type2 = tk.Radiobutton(
        frame1,
        text="游戏环境非(Not) Windows，比如Linux，Android系统",
        variable=check1,
        value=0,
        justify="left",
    )

    entry_metadata_file = tk.Entry(
        frame2, textvariable=file1, state=tk.DISABLED, width=60
    )
    button_metadata_file = tk.Button(
        frame2, text="天马前端数据文件选择pegasus.metadata.txt", command=lambda: selectfile(file1)
    )

    entry_dest_playlist = tk.Entry(
        frame3, textvariable=path2, state=tk.DISABLED, width=60
    )
    button_dest_playlist = tk.Button(
        frame3,
        text="Retroarch Playlist output path 列表文件输出文件夹选择",
        command=lambda: selectpath(path2),
    )

    entry_prefix_label = tk.Label(
        frame4,
        text="Rom path prefix 路径前缀,比如为Android平台制作列表的场景,/storage/1111-1111/roms/PSP,\r\n默认为metadata文件所在目录(可选,Optional)",
    )
    entry_prefix = tk.Entry(frame4, width=60)

    entry_db_name_label = tk.Label(
        frame5,
        text="固定所有列表文件的db_name值及输出列表文件名,比如MAME.lpl,需要和最终retroarch封面所在的文件夹名称一致，\r\n 默认为metadata所在文件夹名称(可选,Optional)",
    )
