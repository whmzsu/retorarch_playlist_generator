...
def get(check1, entry_rom_path, entry_dest_playlist, entry_prefix, entry_db_name):
    choice = check1.get()
        ...
        for root, dirs, files in os.walk(romspathdir):
            if files:
                ...
                playlist = os.path.join(
                    lplpathdir, "07 GBA-"+os.path.basename(root))+".lpl"
...
def main():
    ...
    button_confirm = tk.Button(
        frame6, text="确定 Go", command=lambda: get(check1, entry_rom_path, entry_dest_playlist, entry_prefix, entry_db_name))
